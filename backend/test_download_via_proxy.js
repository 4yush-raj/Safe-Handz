...
async function run() {
  ...
  const req = http.request(options, (res) => {
    ...
    const fileStream = fs.createWriteStream('test_downloaded_via_proxy.pdf');
    res.pipe(fileStream);
    
    fileStream.on('finish', () => {
      fileStream.close();
      console.log('File download via proxy finished.');
      const stats = fs.statSync('test_downloaded_via_proxy.pdf');
      console.log(`Downloaded file size: ${stats.size} bytes`);
      
      // Check first 20 bytes
      const fd = fs.openSync('test_downloaded_via_proxy.pdf', 'r');
